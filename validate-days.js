...
const interactionTypesPath = path.join(__dirname, 'interaction-types.js');
const dataPath = path.join(__dirname, 'data.js');
const context = { window: {} };
vm.createContext(context);
vm.runInContext(fs.readFileSync(interactionTypesPath, 'utf8'), context);
vm.runInContext(fs.readFileSync(dataPath, 'utf8'), context);

const days = context.window.MARRYME_DAYS;
const allowedInteractionTypes = new Set(context.window.MARRYME_INTERACTION_TYPES || []);
