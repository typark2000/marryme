...
## Problem statement
하루에 하나씩 올라오는 장난스럽고 기억에 남는 청혼 웹페이지를 빠르게 공개하고, 날짜가 늘어나도 유지보수 가능한 방식으로 관리할 수 있는 구조가 필요하다.
...
- interaction type별 공통 핸들러 구조 제공
- 이전/다음 day 네비게이션 제공
- GitHub Pages 배포 가능
...
2. 방문자로서, 여러 날의 청혼 디자인을 하나씩 비교해보고 싶다.
3. 방문자로서, 특정 day의 개별 링크를 열어 바로 체험하고 싶다.
4. 제작자로서, 새 디자인을 `data.js` 중심으로 빠르게 확장하고 싶다.
5. 제작자로서, 잘못된 slug/메타데이터를 push 전에 잡고 싶다.
6. 제작자로서, interaction 로직이 day 수가 늘어나도 너무 엉키지 않길 원한다.

## Acceptance criteria
- 메인 화면에 오늘의 디자인이 렌더링된다.
- Day 001 ~ Day 007 데이터가 존재한다.
...
- interaction type별 동작이 각각 정상 작동한다.
